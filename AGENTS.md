...
<project_context>
Project-specific guidance for this repository is defined in `./CLAUDE.md`.
Treat `CLAUDE.md` as the authoritative source for domain context, business vocabulary, stack expectations, and delivery constraints for this Hankyu Hanshin bidding-automation project.
Do not modify `CLAUDE.md` when syncing or extending this AGENTS surface unless the user explicitly requests changes to that file.

Project baseline:
- Build an AI-assisted bidding/tender automation system for Hankyu Hanshin.
- Core flow: parse customer PKG documents, retrieve lane tariffs, fill the bid package, then notify sales for review.
- Primary stack: Python + FastAPI + Celery backend; React + TypeScript frontend; PostgreSQL and optional Redis; Docker deployment.
- Data and document work commonly involve Excel/PDF handling, tariff tables, templates, and multilingual UI/content.

Project execution rules from `CLAUDE.md`:
- Preserve zh/ja/en multilingual behavior. Frontend UI must support language switching through i18n.
- Use Python `snake_case` and React/TypeScript `camelCase`.
- Follow PEP 8 and use type hints in Python code. Prefer function components and Hooks in React.
- Keep API surfaces RESTful with consistent error responses.
- Write code comments and commit messages in Chinese. Write API documentation in English.
- Never hardcode tariff data, credentials, API keys, or database connection secrets. Use environment variables for sensitive configuration.
- Respect the repository structure in `CLAUDE.md` when placing backend, frontend, data, and script changes.

Business vocabulary that should stay consistent:
- `入札`: bidding/tender
- `見積`: quotation
- `費率表`: tariff
- `航線`: lane
...
When OMX general guidance and `CLAUDE.md` both apply, keep OMX orchestration/safety/verification behavior and use `CLAUDE.md` for project-specific product, stack, naming, language, and data-handling rules.
</project_context>

<project_context_encoding_note>
If any Japanese business terms render incorrectly in a Windows terminal, use these canonical aliases:
- `nyusatsu`: bidding/tender
- `mitsumori`: quotation
- `tariff table`: tariff
- `lane`: lane
- `bid package` / `PKG`: bid package
- `sales`: sales
</project_context_encoding_note>
